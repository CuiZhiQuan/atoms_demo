...
## Why
在 **8 小时内**实现一个类 Atoms 的多智能体协作应用生成 MVP：用户用自然语言描述需求，由 AI 团队（团队负责人 / 产品经理 / 架构师 / 工程师）按 SOP 协作生成可预览的 Web 应用。所有核心功能完整交付，时间节奏按 8 小时紧凑编排。
...
- **新增** 应用预览能力（静态文件服务 + iframe 嵌入）
- **新增** 项目持久化（SQLite 存元数据，文件系统存项目代码）
...
## Impact
- **新增 specs**：后端 API、Agent 编排、LLM 客户端、存储、前端 UI、流式协议
- **新增代码目录**：
  - `backend/`（FastAPI 服务）
...
#### Scenario: 前端项目预览
- **WHEN** Engineer Agent 生成 `index.html` 或 `package.json` + `src/`
- **THEN** 后端将该项目根目录挂为静态文件服务
- **AND** 前端通过 iframe 嵌入 `http://<host>:<port>/preview/<project_id>/`
- **AND** 支持移动端 / PC 端视图切换
...
#### Scenario: 项目元数据存储
- **WHEN** 用户首次创建项目
- **THEN** 在 SQLite 中插入记录：`id, name, mode, created_at, updated_at, file_path`
...
### Requirement: 本地启动
系统 SHALL 支持本地直接启动，无需 Docker：后端 `uvicorn` + 前端 `npm run dev`。

#### Scenario: 启动流程
- **WHEN** 用户执行 `uvicorn backend.main:app --port 8000` 和 `npm run dev`（前端）
...
#### Scenario: 配置文件
- **WHEN** 用户首次克隆项目
- **THEN** 复制 `.env.example` 为 `.env` 并填入 LLM API Key
- **AND** README 包含：技术栈介绍、快速启动、模式说明、FAQ
